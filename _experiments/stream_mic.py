    ...
    def run(self):
        ...
        while True:
            try:
                ...
                data = self.medium.get(
                    timeout=None if is_first else self.serialization_settings.timeout)
                if len(data) == 0:
...
class RecordingWavsThread(threading.Thread):
    ...
    def __init__(self, output_medium: OutputMedium, record_seconds, data_settings: DataSettings, serialization_settings: SerializationSettings):
        ...
        self.buffer_count = int(
            self.data_settings.rate * record_seconds / self.data_settings.chunk)
...
def stream_default():
    ...
    p = pyaudio.PyAudio()
    input_stream = p.open(format=pyaudio.paInt16, channels=2,
                          rate=RATE, input=True, frames_per_buffer=CHUNK)
    output_stream = p.open(format=pyaudio.paInt16, channels=2,
...
def stream_between_threads(serialize_to_wav):
    ...
    q = Queue()
    StreamingWavsThread(InputMedium(connector=q), data_settings=data_settings,
                        serialization_settings=serialization_settings).start()
    RecordingWavsThread(OutputMedium(connector=q), record_seconds=3,
                        data_settings=data_settings, serialization_settings=serialization_settings).start()
    q.join()
...
def stream_between_nodes(serialize_to_wav):
    ...
    node1_proc1 = RecordingWavsThread(OutputMedium(ip=ip, port=port1), record_seconds=3,
                                      data_settings=data_settings, serialization_settings=serialization_settings)
    node2_proc1 = StreamingWavsThread(InputMedium(
        ip=ip, port=port1), data_settings=data_settings, serialization_settings=serialization_settings)
...
def stream_between_nodes2(serialize_to_wav):
    ...
    node1_proc1 = RecordingWavsThread(OutputMedium(ip=ip, port=port1), record_seconds=3,
                                      data_settings=data_settings, serialization_settings=serialization_settings)
    node1_proc2 = StreamingWavsThread(InputMedium(
        ip=ip, port=port2), data_settings=data_settings, serialization_settings=serialization_settings)

    q = Queue()  # use intermediate queue as an example
    DummyPipeline(InputMedium(ip=ip, port=port1),
                  OutputMedium(connector=q)).start()
    DummyPipeline(InputMedium(connector=q),
                  OutputMedium(ip=ip, port=port2)).start()
    q.join()
