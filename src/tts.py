...
class TTSProcessor:
    def __init__(self, checkpoint_path, hf_token=os.environ.get("HF_AUTH")):
        files = [os.path.join(checkpoint_path, f) for f in ["config.json", "model.pth", "vocab.json", "speakers_xtts.pth", "reference.wav"]]
        download = not os.path.exists(checkpoint_path)
        for f in files:
            if not os.path.exists(f):
                download = True
        if download:
            Repo.clone_from(url=f"https://user:{hf_token}@huggingface.co/kzipa/kapitza_voice", to_path=checkpoint_path)
