...
class Pipeline:
    def __init__(self, 
        ...
        self.asr = ASRProcessor(hf_token=os.environ.get('HF_AUTH'))
        self.llm = LLMProcessor(os.environ.get("PROMPT_PATH"), os.environ.get("RAG_PATH"), model_url=model_url)
        self.tts = TTSProcessor(os.environ.get("AUDIO_PATH"), hf_token=os.environ.get('HF_AUTH'))
...
if __name__ == '__main__':
    ...
    output_mode = "audio"
    pipe = Pipeline()
    result = pipe.process(user_name=user_name, file_to_process=file_to_process, output_mode="audio")
