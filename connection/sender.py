    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                print(
                    f"Empty queue for {self.timeout} seconds! Server is closing connection.")
            ...
            with open(file_name, 'rb') as f:
                while len(data := f.read(self.chunk)):
                    try:
                        ...
                        print(
                            f"Transmission interrupted by client for file {file_name}. Exception: {e}")
                        # self.server_socket.close()
                        self.queue.task_done()
                        os._exit(-1)
            print(
                f"Finished transmission for file {file_name}. Sent {data_len} bytes")
            self.queue.task_done()
